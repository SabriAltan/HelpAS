...
function sqlClient(env) {
  if (!env.DATABASE_URL) throw new Error("DATABASE_URL yok");
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const sql = env.DATABASE_URL ? sqlClient(env) : null;
      ...
      if (url.pathname === "/api/users" && request.method === "GET") {
        if (!sql) return json({ ok: false, error: "DATABASE_URL yok" }, 500);
        const rows = await sql`
          select username, name, role, wa, active, created_at
          from users
          order by username
        `;
      ...
      if (url.pathname === "/api/equipment" && request.method === "POST") {
        ...
        if (!name || !deviceId) {
          return json({ ok: false, error: "name ve device_id gerekli" }, 400);
        }
        ...
        const rows = await sql`
          insert into equipment (
            ...
            ${name}, ${location || null}, ${cust}, ${brand}, ${model},
